...
\email{scott@ScottsSecondAct.com}
...
limiter) show a mean evaluation latency of 2.4\,\textmu s on the full
evaluation path---a factor of 4{,}166$\times$ under the 10\,ms
per-event budget. A Python SDK with LangChain and OpenAI integrations enables drop-in
...
\begin{itemize}
  ...
  \item \textbf{Evaluation} demonstrating 2.4\,\textmu s mean latency on a
    realistic policy---a factor of 4{,}166$\times$ under the 10\,ms budget
    (\S\ref{sec:eval}).
\end{itemize}
...
\texttt{before($\phi$, $\psi$)} & $\neg\psi \,\mathcal{U}\, \phi$ & $\phi$ must precede $\psi$ \\
\texttt{after($\phi$, $\psi$)} & $\square(\psi \Rightarrow \mathcal{X}\phi)$ & after $\psi$, $\phi$ must hold next \\
...
\begin{description}
  ...
  \item[\texttt{after($\phi$, trigger)}] Four-state machine: $s_0$ (waiting
    for trigger); transition to $s_1$ (triggered) on \textit{trigger};
    from $s_1$, transition to $s_2$ (satisfied, absorbing) on $\phi$, transition
    to $s_3$ (violated, absorbing) on $\neg\phi$. Because $\phi$ is evaluated on
    the event immediately following the trigger, this encodes
    $\square(\textit{trigger} \Rightarrow \mathcal{X}\phi)$.
  \item[\texttt{before($\phi$, $\psi$)}] Three-state machine: $s_0$ (waiting);
    transition to $s_1$ (satisfied, absorbing) on $\phi$; transition to $s_2$
    (violated, absorbing) on $\psi$. Enforces that $\phi$ occurs before $\psi$
    in the event stream ($\neg\psi \,\mathcal{U}\, \phi$).
  \item[\texttt{next($\phi$)}] Four-state machine: $s_0$ (initial); unconditional
    transition to $s_1$ (checking) on the next event; from $s_1$, transition to
    $s_2$ (satisfied, absorbing) on $\phi$, transition to $s_3$ (violated,
    absorbing) on $\neg\phi$.
\end{description}
...
The runtime's \texttt{Value} type is an eight-variant enum covering \texttt{Null},
...
2.4\,\textmu s mean latency on the full evaluation path (compound conditions,
all 10 rules scanned)---a factor of 4,166$\times$ under the 10\,ms budget.
For allow-listed events that exit after the first matching rule, steady-state
throughput on the same policy exceeds 400{,}000 events per second on a single
thread.
...
The \texttt{data\_exfiltration\_guard.aegis} example policy (142 lines) encodes
15 distinct safety properties spanning all six categories above. The
\texttt{research\_guard.aegis} example (98 lines) specifically demonstrates
...
achieving 85.2\% line coverage. Coverage by module:
...
monitors Java execution traces using Quantified Event Automata (QEA) as the
...
Rego (Open Policy Agent~\cite{toews2021opa}) is a Datalog-inspired policy
language for access control. Cedar~\cite{cedar2023} is a purpose-built
attribute-based access control language with SMT-backed static analysis.
Both operate on individual decision requests and do not model event sequences.
AutomaGuard's Aegis provides temporal operators and an agent-specific event
model not present in either.

\paragraph{Formal verification of AI systems.}
DNN verification tools (Marabou~\cite{katz2019marabou},
$\alpha,\beta$-CROWN~\cite{xu2021fast,wang2021betacrown})
verify input-output properties of the function computed by a deployed neural
network, not behavioral policies over agent event streams. AI safety frameworks such as Constitutional AI~\cite{bai2022constitutional}
...
During the preparation of this work, the author used Claude Code (Anthropic) to
assist in the development of the Rust runtime and the drafting of the manuscript.
This tool was used to refine code efficiency, generate initial benchmarking
scripts, and improve the clarity of the abstract and technical descriptions.
Following the generation of these components, the author performed rigorous manual
verification of all mathematical formalisms, code logic, and benchmark results,
and takes full responsibility for the technical accuracy and integrity of the
content.
