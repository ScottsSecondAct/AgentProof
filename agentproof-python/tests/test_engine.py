...
from tests.conftest import make_aegisc_bytes, make_aegisc_file
...
class TestPolicyEngineFromBytes:
    def test_loads_policy_name(self):
        data = make_aegisc_bytes({"name": "bytes-policy"})
    ...
    def test_raises_for_bad_magic(self):
        ...
        with pytest.raises(ValueError, match="Not valid .aegisc data"):
...
class TestPolicyEngineFromJson:
    def test_loads_policy_name(self):
        engine = PolicyEngine.from_json(json.dumps({"name": "json-policy"}))
