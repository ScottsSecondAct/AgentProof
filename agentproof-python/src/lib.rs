    ...
    match val {
        ...
        Value::List(items) => {
            let list = PyList::new_bound(
                py,
                items.iter().map(|v| value_to_py(py, v)),
            );
            list.into_py(py)
        }
        Value::Map(map) => {
...
impl PyPolicyEngine {
    ...
    fn from_file(path: &str) -> PyResult<Self> {
        ...
        let policy = bytecode::read_file(p).map_err(|e| {
            PyRuntimeError::new_err(format!("Failed to load policy: {e}"))
        })?;
    ...
    fn from_bytes(data: &[u8]) -> PyResult<Self> {
        let policy = bytecode::from_bytecode(data).map_err(|e| {
            PyRuntimeError::new_err(format!("Failed to parse policy bytes: {e}"))
        })?;
        let name = policy.name.to_string();
        let engine = RustEngine::new(policy);
        Ok(Self {
    ...
    fn from_json(json_str: &str) -> PyResult<Self> {
        let policy: CompiledPolicy = serde_json::from_str(json_str).map_err(|e| {
            PyValueError::new_err(format!("Invalid policy JSON: {e}"))
        })?;
        let name = policy.name.to_string();
        let engine = RustEngine::new(policy);
        Ok(Self {
    ...
    fn evaluate(
        ...
        let result = {
            let mut engine = self.inner.lock().map_err(|_| {
                PyRuntimeError::new_err("Engine lock poisoned")
            })?;
    ...
    fn event_count(&self) -> PyResult<u64> {
        let engine = self.inner.lock().map_err(|_| {
            PyRuntimeError::new_err("Engine lock poisoned")
        })?;
...
fn result_to_py(py: Python<'_>, result: &RustResult) -> PyObject {
    ...
    dict.set_item("verdict", format!("{:?}", result.verdict).to_lowercase()).unwrap();
    dict.set_item("reason", result.reason.as_deref()).unwrap();
    dict.set_item("triggered_rules", &result.triggered_rules).unwrap();
    dict.set_item("eval_time_us", result.eval_time_us).unwrap();

    // Actions
    let actions = PyList::new_bound(py, result.actions.iter().map(|a| {
    ...
    // Violations
    let violations = PyList::new_bound(py, result.violations.iter().map(|v| {
        let d = PyDict::new_bound(py);
    ...
    let cvs = PyList::new_bound(py, result.constraint_violations.iter().map(|cv| {
        let d = PyDict::new_bound(py);
